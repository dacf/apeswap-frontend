...
const fetchBillsTokenPrices = async (chainId: number, bills: Bills[]) => {
  ...
  const uniV2Tokens: ParsedTokens[] = bills
    ...
    ?.flatMap(({ token, quoteToken, lpToken, lpType }) => {
      return [
        { token, address: token.address[chainId], decimals: token.decimals[chainId], isLp: false, lpType },
        {
          token: quoteToken,
          address: quoteToken.address[chainId],
          decimals: quoteToken.decimals[chainId],
          isLp: false,
          lpType,
        },
        { token: lpToken, address: lpToken.address[chainId], decimals: lpToken.decimals[chainId], isLp: true, lpType },
      ]
    })
  ...
  const tokenPrices = [
    ...uniV2Tokens?.map(({ token, address, decimals }, i) => {
      return { symbol: token.symbol, address, price: getBalanceNumber(tokenV2Prices[i], decimals), decimals }
    }),
    ...uniV3Tokens.map(({ token, address, decimals }, i) => {
      return { symbol: token.symbol, address, price: getBalanceNumber(tokenV3Prices[i], decimals), decimals }
    }),
  ]
